-- ============================================================
-- Migration 210: stamp sucursal_id on COGS egress in crear_venta_atomica
-- ============================================================
-- The COGS egress movimiento_caja (step 7 of crear_venta_atomica) was
-- inserting without sucursal_id. Migration 207 adds NOT NULL on that column
-- so this fix must be deployed before 207 is applied.
--
-- Change: add sucursal_id + p_sucursal_id to the movimientos_caja INSERT
-- in step 7. No other logic, signature, or behavior is changed.
--
-- Uses CREATE OR REPLACE (signature is unchanged — 20 params). No DROP needed.
-- ============================================================
...
COMMENT ON FUNCTION crear_venta_atomica(
  ...
  'Crea venta atómica. v210: agrega sucursal_id al egreso COGS en movimientos_caja. '
  'v206: agrega p_sucursal_id (persistido en ventas.sucursal_id) '
