-- Support deferred payments: allow creating a venta with $0 initial payment ("paga después")
-- Only changes vs 066: the ELSE branch for p_pagos now distinguishes NULL (legacy) vs empty array (deferred)
-- p_pagos = NULL  → backwards compatible: assumes full payment
-- p_pagos = []    → deferred: monto_abonado = 0, estado_pago = PENDIENTE, no payment records
