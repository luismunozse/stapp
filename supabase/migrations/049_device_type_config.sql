-- Agregar columna config JSONB a tipos_dispositivo para personalización por tipo
ALTER TABLE tipos_dispositivo ADD COLUMN IF NOT EXISTS config JSONB DEFAULT '{}';

-- Agregar columna metadata JSONB a ordenes_servicio para campos extra dinámicos
ALTER TABLE ordenes_servicio ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}';

-- Migrar configuraciones hardcodeadas como defaults para tipos base
-- CELULAR
...
}'::jsonb WHERE codigo = 'COMPUTADORA';
...
}'::jsonb WHERE codigo = 'SMARTWATCH';

-- TODOS (genérico/fallback)
UPDATE tipos_dispositivo SET config = '{
  "campos": {
  ...
  "marcas": []
}'::jsonb WHERE codigo = 'TODOS';

-- Para tipos personalizados que ya existen sin config, asignar config genérica
UPDATE tipos_dispositivo SET config = (
  SELECT config FROM tipos_dispositivo WHERE codigo = 'TODOS' LIMIT 1
