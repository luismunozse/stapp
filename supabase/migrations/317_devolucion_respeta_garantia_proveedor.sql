-- ============================================================================
-- 317: no borrar la garantía del PROVEEDOR al devolver
-- ============================================================================
-- Defecto introducido por la 316: al retirar las series de una devolución se
-- ponía `fecha_garantia_vence = NULL` siempre.
--
-- Pero ese campo está sobrecargado. `registrar_series_ingreso`
-- (175_lotes_series.sql:431) lo estampa en el ALTA con la garantía del
-- PROVEEDOR, sobre series DISPONIBLE que todavía no se vendieron. Y la venta
-- solo lo pisa cuando la línea lleva días de garantía:
--
--     fecha_garantia_vence = CASE
--       WHEN v_dias_garantia > 0 THEN CURRENT_DATE + v_dias_garantia
--       ELSE fecha_garantia_vence END
--
-- O sea: si la línea se vendió con 0 días, lo que quedó en el campo es la fecha
-- del proveedor. La 316 la borraba al devolver — dato bueno destruido.
--
-- Fix: limpiar solo cuando la fecha la puso esta venta (dias_garantia > 0).
-- Cuando la línea llevaba garantía, la fecha del proveedor ya se había perdido
-- en la venta misma y no hay nada que preservar.
--
-- Espeja retirarSeriesDeVenta() en app/api/ventas/[id]/devolucion/route.ts.
-- El enum ANULADA ya lo agregó la 316: esta migración NO lleva ALTER TYPE y se
...
DECLARE
  ...
  v_paid_unit           DECIMAL;
  -- Warranty retirement (Step 6 / 6b)
  v_line_sold           INTEGER;
  v_line_returned       INTEGER;
  v_line_warranty_days  INTEGER;
BEGIN
  -- ----------------------------------------------------------------
  ...
  LOOP
    ...
    -- (175_lotes_series.sql:431) lo estampa en el ALTA con la garantía del
    -- PROVEEDOR, y la venta solo lo pisa cuando la línea llevaba días de
    -- garantía (ELSE fecha_garantia_vence). Si la línea se vendió con 0 días, lo
    -- que hay ahí es del proveedor y borrarlo destruye dato bueno.
    SELECT dias_garantia INTO v_line_warranty_days
    FROM items_venta
    WHERE id = elem->>'itemVentaId' AND venta_id = p_venta_id;
    ...
    -- manos del cliente en ambos casos. `fecha_garantia_vence` se limpia solo
    -- si la puso esta venta — si la serie vuelve al stock con la fecha del
    -- comprador anterior, marcar_serie_vendida (175) la relee al revenderla y
    -- la marca GARANTIA_ACTIVA heredando una garantía que nadie compró.
    IF (elem->>'restaurarStock')::boolean THEN
      ...
      SET estado = 'DISPONIBLE', fecha_venta = NULL, venta_id = NULL,
          cliente_id = NULL, fecha_garantia_vence = CASE
            WHEN COALESCE(v_line_warranty_days, 0) > 0 THEN NULL
            ELSE fecha_garantia_vence END, updated_at = now()
      WHERE id IN (
    ...
    ELSE
      ...
      UPDATE inventario_series
      SET estado = 'DEVUELTO', fecha_garantia_vence = CASE
            WHEN COALESCE(v_line_warranty_days, 0) > 0 THEN NULL
            ELSE fecha_garantia_vence END, updated_at = now()
      WHERE id IN (
        SELECT id FROM inventario_series
...
COMMENT ON FUNCTION public.registrar_devolucion_atomica(TEXT,TEXT,TEXT,TEXT,TEXT,TEXT,TEXT,TEXT,JSONB) IS
  ...
  'Migración 317: fecha_garantia_vence solo se limpia si la línea se vendió '
  'CON garantía — con 0 días la fecha es la del proveedor (alta de serie) y '
  'borrarla destruía dato bueno.';
