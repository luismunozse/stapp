-- ============================================
-- Migration 206: crear_venta_atomica con sucursal_id
-- ============================================
-- Sistema multi-sucursal: las ventas POS se crean vía la RPC atómica
-- crear_venta_atomica. La tabla ventas ya tiene la columna sucursal_id
-- (nullable, agregada en migración 202). Esta migración agrega el parámetro
-- p_sucursal_id y lo persiste en el INSERT INTO ventas.
--
-- Reproducida VERBATIM desde la def vigente (migración 200, líneas 48-422),
-- con SOLO 2 cambios:
--   (1) Nuevo último parámetro: p_sucursal_id TEXT DEFAULT NULL.
--   (2) En INSERT INTO ventas: columna sucursal_id + value p_sucursal_id.
-- Ningún otro cambio en la lógica, variables, manejo de errores ni INSERT/UPDATE.
--
-- La firma vigente (200) tiene 19 params; agregar uno con DEFAULT crea un
-- overload distinto. Igual que en migración 200 (y el bug de la mig 055),
-- se dropea la firma vigente por nombre+tipos exactos antes de recrear, para
-- que PostgREST no quede con dos overloads ambiguos.
-- ============================================

-- Dropear la firma vigente de 200 (19 params) por tipos exactos.
...
COMMENT ON FUNCTION crear_venta_atomica(
  ...
  'Crea venta atómica. v206: agrega p_sucursal_id (persistido en ventas.sucursal_id) '
  'sobre la def v200 (consume inventario_series inline para items con trackea_series '
  '— auto FIFO u override por serieIds, diasGarantia recalcula garantía de la serie; '
  'persiste idempotency_key, único por org).';
