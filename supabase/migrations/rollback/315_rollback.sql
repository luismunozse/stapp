...
--
-- CUIDADO: no es un rollback limpio de DATOS. Las reservas que la 315 ya haya
...
--
-- 2. Liberarlas TODAS mientras las funciones de la 315 siguen vivas:
--
--      SELECT liberar_reserva_catalogo(c.id, 'Rollback 315')
--      FROM cotizaciones c
--      WHERE c.origen = 'CATALOGO_PUBLICO'
--        AND EXISTS (SELECT 1 FROM reserva_cotizacion_pendiente(c.id));
--
...
--     convertir-venta.
--
-- NO hace falta revertir (son correctas por si mismas y no dependen de la 315):
...
-- hicieron. Dropearla y volver a aplicar la 315 no reconstruye nada, pero
