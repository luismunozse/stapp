...
// POST /api/inventario/[id]/kit/desensamblar
// Wrapper de desensamblar_kit_atomic.
export async function POST(
  ...
  try {
    ...
    if (rpcErr) {
      ...
      const msg = rpcErr.message || "Error al desensamblar kit"
      if (code === "P0002") {
    ...
    console.error("Error desensamblando kit:", err)
    return NextResponse.json(
      { error: "Error al desensamblar kit" },
      { status: 500 }
    )
  }
}
