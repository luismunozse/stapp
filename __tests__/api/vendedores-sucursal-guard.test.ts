/**
 * Tests: sucursal_id guard on POST /api/vendedores
 *
 * Scenarios:
 *  VG.1 — org has no principal sucursal: returns 400, no insert
 *  VG.2 — org has principal sucursal: returns 201, insert includes sucursal_id
 */
...
describe("POST /api/vendedores — sucursal_id guard", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it("VG.1 — returns 400 and skips insert when org has no principal sucursal", async () => {
  ...
  it("VG.2 — returns 201 and insert includes sucursal_id when principal exists", async () => {
    ...
    // then insert+select returns the new vendedor for the second call.
    ...
    vi.mocked(supabaseAdmin.from).mockImplementation((table: string) => {
      ...
      if (table === "users") {
        ...
        // second call: insert + select → new vendedor
